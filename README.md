...
| 43 | 도움말 창 기능 | 완료 | 상단 `?` 버튼을 누르면 Tub Manager와 Trainer 사용 방법을 안내하는 별도 Help 창이 표시되도록 구현했습니다. |
...
상단 우측의 `?` 버튼을 누르면 프로그램 사용 방법을 안내하는 도움말 창이 표시되도록 구현했습니다.

- 메인 화면 상단의 `?` 버튼을 통해 도움말 창을 열 수 있습니다.
- 도움말 창에는 Tub Manager, Trainer, Help Window 사용 방법이 구분되어 표시됩니다.
- Tub Manager 영역에서는 폴더 열기, 이미지 미리보기, 프레임 삭제, 삭제 로그 관리, 재생 컨트롤, Tub Cleaner, Filter 기능을 안내합니다.
- Trainer 영역에서는 차량 폴더 선택, 모델 선택, 학습 실행, Training Loss Chart, 모델 관리 기능을 안내합니다.
- 도움말 창은 메인 화면 위에 표시되며, 사용자가 프로그램 기능을 보면서 동시에 도움말을 참고할 수 있습니다.
- 상단 제목 영역을 드래그하여 도움말 창의 위치를 이동할 수 있습니다.
- 우측 상단 `X` 버튼을 누르면 도움말 창을 닫을 수 있습니다.

이 기능을 통해 사용자는 별도의 문서를 열지 않아도 프로그램 안에서 주요 기능의 사용 방법을 바로 확인할 수 있습니다.
