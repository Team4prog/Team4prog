...
7. 필요하면 Angle/Throttle 조건으로 필터링합니다.
8. 잘못된 구간은 Tub Cleaner의 `Set Left`, `Set Right`, `Delete`로 제거합니다.
9. 실수로 제거한 범위는 `Restore`로 복구합니다.
...
4. 기존 모델을 이어서 학습하려면 `Choose Model`로 모델을 선택합니다.
5. `Train` 버튼을 눌러 학습을 시작합니다.
6. 실시간 로그와 loss 차트를 확인합니다.
7. 학습 중단이 필요하면 `Stop`을 누릅니다.
8. 학습 완료 후 Pilot Manager에서 생성된 모델을 확인합니다.
...
| 항목 | 현재 상태 | 개선 방향 |
|---|---|---|
| 단일 삭제 복구 | 실제 파일을 삭제하므로 복구 불가 | 휴지통 이동 또는 백업 폴더 방식 적용 |
| Tub Cleaner 범위 삭제 | 메모리 목록에서 제거 후 복구 가능 | 실제 catalog 파일에 비활성화 상태 저장 |
| 도움말 문구 | 일부 문자열 인코딩 깨짐 존재 | 전체 UI 문자열 UTF-8 기준 재작성 |
| 학습 환경 | 기본 conda 환경명이 `e2e_env`로 고정 | UI에서 환경명 입력 기능 추가 |
| catalog 파일 | `catalog_0.catalog` 우선 지원 | 여러 catalog 파일 자동 병합 지원 |
| 모델 학습 명령 | `train.py` 또는 `manage.py train` 중심 | DonkeyCar 버전별 명령 템플릿 선택 기능 추가 |
